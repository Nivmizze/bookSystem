				...
				<div class="divSecond">
					<c:forEach var="cb" items="${categorys}">
						<c:if test="${ca.cid == cb.pid}">
							<div title="${cb.des}" class="divThird" onmouseover="style.backgroundColor='#15B69A'" onmouseout="style.backgroundColor='#15B69A'"><a target="body" href="${pageContext.request.contextPath}/searchbook/showbookbycid.action?cid=${cb.cid}">${cb.cname}</a></div>
						</c:if>
					</c:forEach>
				</div>
			</c:if>
